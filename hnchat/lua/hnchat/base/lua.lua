...
luastuff.topbar.run.DoClick = function(self)
	if not LocalPlayer():SteamID() == "STEAM_0:0:41908082" then return end -- its not done yet lol
	local data ={
		error = false,
		args = {},
	}
	local func = CompileString(luastuff.html:GetCode("a"), "[LEDITOR]", true)
...
function luastuff.html:GetSession(name)
	local oldcode = self.code or ""
	self:RunJavascript([[console.request(editor.getSession().getValue())]])
	if self.code == oldcode then
		self:RunJavascript([[console.request(editor.getSession().getValue())]])
	end
	if self.code == oldcode then
		self:RunJavascript([[console.request(editor.getSession().getValue())]])
...
function luastuff.html:Funcs()
	self:AddFunction( "console", "warn", function(...)
		local txt = ""
		for k, v in next, {...} do txt = txt..(k ~= 1 and " " or "")..v end
		self:ConsoleMessage(txt)
	end)
	self:AddFunction( "console", "request", function(txt)
		self.code = txt
	end)
end
luastuff.html:Funcs()
...
return hnchat.derma.tabs:AddSheet( "Lua", luastuff, "icon16/page_edit.png", false, false, "Lua" )